...
简单来说就是系统监控体系的统称
   ...
   ### CICD
   CI: 持续集成,每次代码合并都会触发自动化构建和测试流程。<br/>
   CD： 持续部署，是持续交付的延伸，意味着每一个经过验证的变更都可以自动部署到生产环境，无需人工干预<br/>
   ### cpu
   一台机器的cpu正常只有一两个，但是核心数一般很多个。即一个cpu有多个核心，一般是通过核心数来判断的
   ### MFA
   MFA多因素认证（Multi-Factor Authentication）
定义：多因素认证是一种电子认证方法，要求用户在登录或进行其他交易时，提供来自两个或多个独立类别的凭据来验证其身份。它不再仅仅依赖传统的用户名和密码组合，而是增加了额外的验证因素，从而提高了账户的安全性
   ### pilot
   在互联网或科技领域中，"pilot"（试点） 通常指的是一种小规模、有限范围的测试或实验性部署，用于验证某个新功能、新产品或新系统的可行性、稳定性或用户反馈。
   ### csv
   CSV 是一种纯文本格式，一种用 逗号分隔的纯文本文件，用于表示表格数据。每行代表一条记录，每个字段用逗号 , 分开
...
## Java
   ...
   ### 类型强转
   父类强转子类是会报错的。除非一开始这个父类就是指向的子类对象
   ### 小数计算
   double与float进行运算时，会有精度问题！最后的几位小数不一定准确。所以对于汇率等要求高精度的，不要使用double或float做运算，可以考虑用BigDecimal的<br/>
比如，0.1 会被表示为 1 和指数 -1（即 1 * 10^(-1)），而 0.2 会被表示为 2 和指数 -1（即 2 * 10^(-1)）。进行加法时，它们会变成整数，最终计算得到的结果也能精确表达。
   ### 泛型
   泛型在编译成class文件时，会进行一个类型转换，将T的类型转成父类，也就是说涉及list<xx>.class文件中基本都是List Object这种格式的
   ### 抽象类中的构造函数
...
在 Linux 和容器环境中，_JAVA_OPTIONS 是一个被 Java 虚拟机（JVM）内核底层直接识别的特殊环境变量。 在 Docker 或 Kubernetes (K8s) 环境中，如果你无法修改容器镜像内部的启动命令（Command/Args），你只需要在 Pod 或容器的 env 配置中添加 _JAVA_OPTIONS，就能强行控制该容器内所有 Java 进程的堆内存（如 -Xmx）、垃圾回收策略（GC）或系统属性（-D）。
   ...
   ### lambda
   在lambda表达式中，如果使用外部的变量，必须是final或者没有修改过的。不然会造成歧义，因为实际lambda中操作的是堆对象，是一个固定的<br/>
   lambda使用局部变量时，方法种的局部变量必须是final的，因为lambda的生命周期可能长于方法，所以lambda中其实会有个局部变量的副本，在堆中，因此如果这个局部变量被重新赋值，lambda感知不到，可能会对开发造成误解。
但是对于成员变量和静态变量，就不用final了，因为生命周期是一样的 <br/>
   ### ClassPath
   ...
   ### 静态内部类与内部类
   https://kimi.moonshot.cn/chat/ctr37tlkfv3q1mum90fg
   static关键字作用于类上时，一定是作用在内部类<br/>
   静态内部类的实例可以不依赖于外部类的实例来创建。你可以直接通过外部类的类名来创建静态内部类的实例。静态内部类只能访问外部类的静态成员（静态字段和静态方法）。它不能访问外部类的非静态成员，因为静态内部类的实例与外部类的实例无关。<br/>
   非静态内部类的实例必须依赖于外部类的实例来创建。你需要先创建外部类的实例，然后才能创建内部类的实例。 非静态内部类可以访问外部类的所有成员，包括非静态字段和方法。这是因为非静态内部类的实例与外部类的实例相关联<br/>
   比如创建一个实体类来做db的映射的话，一般是创建静态内部类。可以理解为是独立于外部类的一个类
   ### annotation:
1.  Java 标准的 @Inherited 并不直接支持方法级别的继承。也就是说，如果子类重写（Override）了父类的方法，子类的方法上是不会自动带上这个注解的。
2. 注解有个@Retention(RetentionPolicy.RUNTIME)配置，这个代表着注解要保留到什么阶段，
RetentionPolicy.SOURCE:注解仅在源代码中存在，编译时会被丢弃。适用于那些只在编译时需要的信息，如生成代码时的帮助信息。
RetentionPolicy.CLASS:注解保留在 .class 文件中，但在 JVM 加载类时会丢失。默认的行为，适用于大多数不需要在运行时反射的注解。
RetentionPolicy.RUNTIME:注解不仅保留在 .class 文件中，而且在程序运行时也能通过反射读取。适用于需要在运行时获取注解信息的场景，像上面提到的 AOP（切面编程）就需要这种注解。
   ### pair
...
jdbc:jdbc有两种注册方法，
第一种在代码里直接调用Class.forName("xx"),这种会由当前类的类加载器来加载。不会有问题<br/>
第二种是通过spi,第三方包在META-INF里面声明具体的实现。这种方式会在DriverManager(由bootstrap类加载器)init的时候去调用Class.forname但是如果是双亲委派机制的情况下，会找不到这个class(Class.forName("xx"),会由当前类的类加载器来加载)。所以DriverManager里面加了个Thread.currentThread().getContextClassLoader()方法，获取当前线程的类加载器(main线程，会是application类加载器)。所以可以加载到。
不到的class文件<br/>
#### 内存分配
...
## MAVEN
   ...
   ### pom.xml
    pom中的repository的id,会与maven setting中配置的密码对应。所以不可以乱写
...
## 环境
### sit
系统集成测试，由测试或者开发在这个环境进行测试
### UAT
用户验收测试，主要是由用户在这个环节进行测试
### reg
回归测试环境，用于跑回归测试的地方。之所以不在UAT上跑回归测试而是建一个单独的环境，主要是为了隔离。reg环境比较稳定，不怎么进行代码变动。尤其是测试期间
   ## 限流算法
### 固定窗口
顾名思义，设置一个固定的窗口时间，然后定时重置。 但是这样子可能会有边界问题，导致流量超载
###  滑动窗口
####： 将窗口再分成更小的格子(如一秒分成10个格子)，当请求来的时候，判断应该到哪个小格子，哪个小格子就+1计数，然后统计所有格子的计数。 其中实现的原理是在初始时就建好10个格子，然后请求来的时候计算应该放到哪个格子，然后判断这个格子是不是旧的，旧的话就清空。
####： 同时维护每个请求的发送时间，然后统计的时候去遍历(效率比较低，基本不用)
但是都有个问题，第一个是格子如果分的很细，就会占内存比较多，而且无法做到平缓的流量。假设限流 100 QPS，窗口划分为 10 个格子（每格 100ms）。流量不均：在某 1 秒的最开头 100 毫秒（第一个格子）内，瞬间涌入了 100 个请求。由于此时整个大窗口的总数是 0，这 100 个请求会在 100ms 内全部被放行。
### 漏桶算法
桶算法的精髓在于强行平滑输出流量。它的形象隐喻是一个底下打了个孔的无底水桶。
以sentinal为例，每次请求过来的时候，计算下一个允许的时间。也就是说把重心放在每一个请求允许的间隔。然后等新的请求进来了，看看当前时间允不允许，允许的话就去cas替换掉下个预期时间。sentinal这里没有用blocking queue,而是在每个请求自己的线程去尝试cas与sleep.这样做的原因是减少线程的切换。 而且网关要做的是快速判断一个请求要不要被拒绝掉，因此不应该用blocking queue去储存每个请求
### 令牌桶
令牌桶是对漏桶算法的一种改进。它既能保护系统，又保留了应对突发流量的弹性，是 Google Guava RateLimiter 的核心算法。
它的设计哲学是**“相信下游具有一定的弹性伸缩和瞬间抗压能力”。它利用了硬件（CPU、线程池、数据库）的缓冲（Buffer）效应**。下游系统在面对 1 毫秒内涌入的 100 个请求时，虽然瞬时压力暴增，但可以通过操作系统和应用层的连接池队列、线程池队列将其快速消化掉，从而让用户在最短的时间内拿到响应。
## mybatis
   ### retruntype map
...
## 计算机网络
   ...
   #### 公钥与私钥
   公钥与私钥不是依靠加解密来区分的。在ssl中公钥是用来加密的，验证证书时公钥是解密的。
   #### 证书
   颁发的证书一般有三部分： 公钥，私钥，证书，其中证书一般与公钥是在一起的 <br />
   颁发的私钥一般是加密的，需要解密后才可以使用 <br/>
   浏览器内置了很多CA的公钥，通过这些公钥来判断证书是否由他们颁发
   #### 证书签名
   私钥的输入是：哈希值 <br/>
   CA 使用私钥对“证书内容的哈希值”进行加密（即签名）；<br/>
   得到签名（Signature）；<br/>
   这个签名被写入证书。<br/>

   🔓 公钥的输入是：签名（即加密后的哈希）<br/>
   浏览器或客户端用 CA 的公钥对签名进行解密；<br/>
   得到签名前的原始哈希； <br/>
   再对证书内容自己算一次哈希； <br/>
   比较两个哈希是否一致  <br/>
   #### ssl过程
...
## Redis
   ...
   原理是在多台机器申请锁，超过半数则认为申请成功<br />
   redis红锁的使用场景，需要有至少五台master服务.而且其实红锁也无法避免问题。<br />
红锁的核心逻辑是“过半数节点（N/2 + 1）加锁成功才算成功”。假设你部署了 5 个完全独立的 Redis 节点。线程 A 成功在 1、2、3 号节点加锁，拿到了红锁。极端场景：此时 3 号节点突然断电重启（没有配置持久化，或者 AOF 没来得及刷盘）。重启后，3 号节点上的锁数据丢失了。这时候，线程 B 进来加锁，它去 3、4、5 号节点加锁，由于 3 号没有锁数据，4 和 5 也是空闲的，线程 B 也能在 3 个节点加锁成功，同样拿到了红锁。<br />

如果业务看重高性能，允许极端情况下的锁失效：继续用 Redis（通常用普通的 Sentinel 或 Cluster 架构 + Redisson 即可），根本不用红锁。因为红锁维护 5 个独立 Master 的成本太高，且性能很差。针对极少数失效引起的并发，通过数据库的唯一索引或**乐观锁（version）**做最终的兜底防护。<br />
如果业务看重强一致性，绝对不允许锁失效（如金融对账）：直接放弃 Redis，改用基于 standard Raft/Paxos 协议的 ZooKeeper 或 etcd。因为它们在节点写入时必须有一半以上节点强同步成功并落盘，且节点宕机后选出的新主绝对不会丢失数据，从根源上杜绝了主从切换或重启丢锁的可能。<br />
   ### list数据结构
   ...
   ### 跳表
   zset根据score排序时使用<br/>
   https://www.bilibili.com/video/BV1QK4y1Y7mS/?spm_id_from=333.337.search-card.all.click&vd_source=648acd0f84093572b84e9f08a560139e<br/>
   跳表是一个多级索引，最底层是整个数据链表，上一级是去掉一半个数的一个链表，再上一级又是一半，以此类推，类似于二分查找的模式.之所以不用二分查找，是因为二分查找只适用于数组，链表是不支持的<br/>
   ### redis架构
   主从/哨兵： 单个主节点，多个从节点
   集群： 每个都是主节点，由互为从节点
   ...
   ### redis与mysql一致性
如何保证redis改了，更新mysql成功呢？<br />
可以采用事务消息，先往mq发半消息，然后更新redis，然后再提交给mq给消费者更新MySQL. 这样即使rollback那里失败了，也会去有重试机制
   ### rdb
   存储快照，一般是按时间进行一次存储，比如900秒有一次key操作则rdb.rdb是后台进程，不会阻塞正常命令，也可以用BGSAVE手动rdb
## mongodb
   ### mongodb索引
   使用的b树。 b树与b+树的主要区别，是根节点存不存数据，以及叶子节点支不支持遍历。<br/>
   b树根节点会存值，所以一个根节点能存的key也会少，导致树的高度会增加。但是如果找到了可以直接返回，不需要走到叶子节点.所以如果顺利的话，查询会更快。但是相应的，一页中，索引的数量会降低，那么总页数可能更多了。
...
mongodb是可以直接跟ldap去绑定的，在配置文件中绑定ldap服务器即可。在dbs, mongodb的账号本身就是个ldap用户，锁死等情况都是配置在ldap里的<br/>
## zookeeper
   ### 节点类型
...
## ELK
   ...
   #### msg与keyword
当你没有手动指定复杂的索引结构（Mapping）时，ES 只要看到日志里有文本类型的字段，默认就会把一个文本字段拆成双字段（Fields）模式 <br/>

1. message 字段默认类型：text特性：分词。它会把一长串日志（如 "Received task FAILED"）切成一个个独立的单词（"received", "task", "failed"）并全部变成小写。这样你搜其中任何一个词都能搜到，专门用来做全文检索。<br/>
2. message.keyword 字段默认类型：keyword特性：不分词。它是 message 衍生出来的子字段，会把整条日志当作一个完整的、不可分割的字符串原封不动地保存下来。它区分大小写，通常专门用来做精确匹配、聚合（Aggregation）或排序（Sort）。 <br/>
   #### 集群
   一台机器可以有多个主分片
...
## mysql
   ...
   ### buffer pool
   innodb在查询时，会把一些页缓存在buffer pool里面，以加速查找。可以通过提升buffer pool大小来增加效率.<br/>
   如果某一页被update,并不会马上被移除，而是会标记为脏页，等待lru时移除.这里主要是因为与redo log相互搭配，所以采用了更新buffer而不是废弃buffer的操作<br/>
   ### mysql语句执行顺序
...
1. InnoDB 存储引擎层：引擎层没有 LIMIT 的概念。它只知道按照条件（user_id = 10086）和顺序，通过 B+ 树一条一条地去磁盘或 Buffer Pool 里读数据。因为你写的是 SELECT *，引擎层每读到一条满足条件的索引，就必须拿着主键 ID 去聚簇索引里回表拿到整行数据。引擎层把这整行数据打包，传送给 Server 层。这个动作要重复 1000010 次！<br />
2.  MySQL Server 层：Server 层专门负责过滤和统计。它接收到引擎层传上来的第 1 条、第 2 条……直到第 1000000 条数据，发现这 100 万条都不在 LIMIT 的区间内。Server 层直接把这 100 万条数据丢弃（扔掉）！直到第 1000011 到 1000010 条，Server 层才保留下来，最后返回给客户端。 <br />
如果需要跳页怎么办 <br/>
1. 限制总页数 <br/>
2. 将深分页的放到es里面 <br/>
   ### mysql的读锁与写锁
读锁只能加在select上 (默认select是不加锁的)，update是只能加写锁。<br />

读未提交（RC） / 读已提交（RC） / 可重复读（RR，默认）：不加锁，通过 MVCC 读取快照，也叫做快照读
串行化（SERIALIZABLE）：会自动加共享锁（S 锁）。此时普通 SELECT 会退化为当前读，如果其他事务在修改这些数据，查询将会被阻塞。
   ### 建立索引
sql查询时，如果根据某个字段去做排序，那么这个字段要加到索引里。且应该跟其他字段建复合索引，放到最右边
   ### ON DUPLICATE KEY
...
## nginx
   ...
   ### 证书
   代理服务器会将客户端的请求转发到目标服务器，并向客户端返回目标服务器的响应，但通常会将证书替换为代理服务器自己的证书。这种方式下，你看到的证书是代理服务器的，而不是目标服务器的。
...
## agGrid
### 简介
ag-Grid 是一个功能强大、高性能的前端数据表格组件库，广泛用于企业级 Web 应用中。支持 React、Angular、Vue、纯 JS 等多种框架，提供 虚拟滚动、大数据量渲染、排序筛选分组、编辑、透视、树形数据 等高级功能。
### 透视
透视列 在表格中想要“转成列来显示”的字段。（将原来某一列的所有值，每个值单独生成一列）
...
```
   ...
   如果懒加载的Bean是通过自动装配（autowiring）注入的，那么在注入它的Bean创建时，懒加载的Bean会被初始化。
可以将需要懒加载的autowired的地方，再添加init（可以理解为autowired的lazy init）
   <br/>
   ...
   #### 注意
   aop只能切bean,自己new的不能代理<br/>
   Spring AOP 是 基于代理对象工作的，不会在内部方法调用中生效。即不能对于 子类内部调用父类方法的父类方法加切点<br/>
   ### controller
   controller的参数，如果是对象的话，要保证相关属性都定义了，没定义的属性是接收不到传参的
   ### bean扫描
   spring启动时默认只会扫描启动类及其子包。如果在第三方pom里面有声明bean,默认会扫描不到的。所以需要手动配置扫描
...
## kafka
   ...
   early: 从最早的未消费的开始消费 
   ### 序列化
   kafka直接传输对象时，需要实现序列化，并且包名不可以变。
   ### 模型
   Kafka 的消费者是“pull 模型”的消息系统，poll() 就是执行“pull”的方法。<br/>
   kafka的poll是这样的，拿到消息后去process.所以如果process过久的话，会导致下一次poll迟迟无法进行。当到达max.poll.interval.ms设置的时间后，会被踢出。 踢出以后会自动尝试重连。但是可能造成消息重复消费的问题 <br/>
...
## 场景题
### 实现一个24小时的排行榜展示
1. 使用flink来做窗口的维护(flink天生支持滑动窗口)，数据先到flink,然后设置每隔多久发到redis的zset里(每次发送的都是最终结果) <br/>
2，通过分桶操作，比如说把每5分钟作为一个桶，把所有这五分钟的统计下来，然后每隔个10来分钟，利用redis的ZUNIONSTORE操作求一次sum
### 元空间占满并且频繁full gc
大概率是有地方在一直动态生成代理对象，或者一直在热加载，而引用没有释放
...
9. 不要有魔法值 <br />
   ...
   #### 查询相关
   分页时利用子查询来优化查找<br />
   使用count(*)而不是count(列名)，第二种会导致忽略掉null值的列<br />
   尽量避免select * ,而是要指定返回的列<br />
   任何查询或者insert等，都要考虑下需不需要分批
   ### cr技巧
